...
main() {
    ...
    if [ -d $HOME/.oh-my-zsh ]; then
        echo -e "${FMT_GREEN}oh-my-zsh already exist.${FMT_RESET}"
    else
        echo -e "${FMT_RED}start install oh-my-zsh. ${FMT_RESET}"
    ...
    if [[ -d "$HOME/powerlevel10k" ]]; then
        echo -e "${FMT_GREEN}powerlevel10k already exist .${FMT_RESET}"
    else
        echo -e "${FMT_RED}start install powerlevel10k. ${FMT_RESET}"
    ...
    if [[ -d "$HOME/.oh-my-zsh/custom/plugins/zsh-syntax-highlighting" ]]; then
        ...
        echo -e "${FMT_RED}start install zsh-syntax-highlighting. ${FMT_RESET}"
        git clone https://github.com/zsh-users/zsh-syntax-highlighting.git ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-syntax-highlighting
    fi
    if [[ -d "$HOME/.oh-my-zsh/custom/plugins/zsh-autosuggestions" ]]; then
        echo -e "${FMT_GREEN}zsh-autosuggestions already exist .${FMT_RESET}"
    else
        echo -e "${FMT_RED}start install zsh-autosuggestions. ${FMT_RESET}"
    ...
    echo -e "${FMT_GREEN}updated plugins in .zshrc.${FMT_RESET}"
